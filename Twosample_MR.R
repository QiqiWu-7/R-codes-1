...
input_path <- "D:/UKB_data/task/SI_outcome_wide/MR/all/all_snp"
...
SI_exp_dat <- read.csv("D:/UKB_data/task/SI_outcome_wide/MR/all/SI_snp final.csv")
mr_method_list()  
...
for (i in folders ) {
  ...
  if (all(name1 %in% names(data))){
    tryCatch({
      ex_out_mer<- merge(SI_exp_dat,data,by.x="SNP",by.y="rsids")
      ex_out_mer$pval <- round(ex_out_mer$pval,3) 
      ex_out_mer_re <- subset(ex_out_mer,pval>=0.05)          
      name<- paste0("SI-e6-",basename (i),".csv")
      out <- file.path("C:/Users/wqq/AppData/Local/R/win-library/4.3/TwoSampleMR/MR/outcome", name )
      write.csv(ex_out_mer_re,out,row.names = FALSE)
      name <- paste0("MR/outcome/",name)
      ...
      dat1 <- harmonise_data(exposure_dat=SI_exp_dat, outcome_dat=outcome_exp_dat1)
      
      res0<- generate_odds_ratios(mr_res = mr(dat1,method_list = c("mr_ivw_mre","mr_weighted_median")))
      
      MRoutcome0 <- data.frame("method"=res0$method,"nsnp"=res0$nsnp,"OR"=res0$or,"CI_low"=res0$or_lci95,
                               "CI_up"=res0$or_uci95,"P"=res0$pval,"outcome"=basename (i),"exposure"="Social isolation")

      MRoutcome_modified0 <- MRoutcome0%>%
        dplyr::mutate(OR= sprintf("%0.2f", as.numeric(OR)),
                      CI_low= sprintf("%0.2f", as.numeric(CI_low)),
                      CI_up= sprintf("%0.2f", as.numeric(CI_up)),
                      P= sprintf("%0.3f", as.numeric(P)))
      MRoutcome_modified0$blank<-rep("")
      MRoutcome_modified0 <- MRoutcome_modified0 %>% unite(CI,`OR`,`CI_low`,sep="(",remove=F)
      MRoutcome_modified0 <- MRoutcome_modified0 %>% unite(CI,`CI`,`CI_up`,sep="-",remove=F)
      MRoutcome_modified0 <- MRoutcome_modified0 %>% unite(CI,`CI`,`blank`,sep=")",remove=F)
      ...
      presso<- mr_presso(BetaOutcome ="beta.outcome", BetaExposure = "beta.exposure", SdOutcome ="se.outcome", SdExposure = "se.exposure", 
                         OUTLIERtest = TRUE,DISTORTIONtest = TRUE, data = dat1, NbDistribution = 10000, SignifThreshold = 5e-6)  
    ...
    tryCatch({
      ex_out_mer<- merge(SI_exp_dat,data,by="SNP")
      ex_out_mer$p_value <- round(ex_out_mer$p_value,3) 
      ex_out_mer_re <- subset(ex_out_mer,p_value>=0.05)
      name<- paste0("SI-e6-",basename (i),".csv")
      out <- file.path("C:/Users/wqq/AppData/Local/R/win-library/4.3/TwoSampleMR/MR/outcome", name )
      write.csv(ex_out_mer_re,out,row.names = FALSE)
      name <- paste0("MR/outcome/",name)
    ...
    tryCatch({
      ...
      ex_out_mer_re <- subset(ex_out_mer,pval.outcome>=0.05)
      name<- paste0("SI-e6-",basename (i),".csv")
      out <- file.path("C:/Users/wqq/AppData/Local/R/win-library/4.3/TwoSampleMR/MR/outcome", name )
      write.csv(ex_out_mer_re,out,row.names = FALSE)
      name <- paste0("MR/outcome/",name)
    ...
    tryCatch({
      ...
      ex_out_mer_re <- subset(ex_out_mer,P>=0.05)
      name<- paste0("SI-e6-",basename (i),".csv")
      out <- file.path("C:/Users/wqq/AppData/Local/R/win-library/4.3/TwoSampleMR/MR/outcome", name )
      write.csv(ex_out_mer_re,out,row.names = FALSE)
      name <- paste0("MR/outcome/",name)
    ...
    tryCatch({
      ex_out_mer<- merge(SI_exp_dat,data,by.x="SNP",by.y="variant_id")
      ex_out_mer$p_value <- round(ex_out_mer$p_value,3) 
      ex_out_mer_re <- subset(ex_out_mer,p_value>=0.05)
      name<- paste0("SI-e6-",basename (i),".csv")
      out <- file.path("C:/Users/wqq/AppData/Local/R/win-library/4.3/TwoSampleMR/MR/outcome", name )
...
write.csv(result,file = "D:/UKB_data/task/SI_outcome_wide/total_results-e6_0.001.csv",row.names = F)
write.csv(Q_result,file = "D:/UKB_data/task/SI_outcome_wide/total_Q-results-e6_0.001.csv",row.names = F)
write.csv(ple_result,file = "D:/UKB_data/task/SI_outcome_wide/total_ple-results-e6_0.001.csv",row.names = F)
write.csv(combined_data,file = "D:/UKB_data/task/SI_outcome_wide/total_presso-results-e6_0.001.csv",row.names = F)
